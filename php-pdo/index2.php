...
try
{
$dbb=mysqli_connect('mysql:host=localhost;dbname=todolist;charset=utf8';'root','root');
}
catch(Exception $errors)
{
	// En cas d'erreur, on affiche un message et on arrête tout
        die($errors : .$errors->getMessage());
}
/*vérifie si on a cliqué sur le bouton "ajouter".*/

if(isset($_POST['submit'])&& $_Post['submit']="Enregistrer"){
  $AFAIRE=$_POST[' A FAIRE'];
    if(empty($AFAIRE)){
      $errors="You must fill in the AFAIRE";
    }else{
        mysqli_querry($dbb,"INSERT INTO AFAIRE(AFAIRE) VALUES('$AFAIRE')");
        header('lacation:index2.php');
        }

        //delete AFAIRE
if(isset($_GET['del_AFAIRE'])){
   $id=$_get['del_AFAIRE'];
   mysqli_querry($dbb,"DELETE")
}
$AFAIRE=mysqli_querry($dbb,"SELECT*FROM AFAIRE");
              ...
              <label>
                <?php
                    while ($donnees = $resultat->fetchAll()) {
                      echo "<input type='checkbox' name='afaire[]' value='".($donnees['nomtache'])."'checked disabled/>
